...
add_action('widgets_init', 'list_posts_init');
function list_posts_init() {
...
if ( ! class_exists( 'List_Posts' ) ) {
	...
	class List_Posts extends WP_Widget {
		function List_Posts() {
			$widget_ops = array(
				'classname'		=> 'list_posts',
				'description'   => __( 'List posts as list' )
			);

			parent::__construct( 'List_Posts', __( 'List Posts' ), $widget_ops );
		}
		...
		function form( $instance ) {
			$instance = wp_parse_args( (array) $instance, array( 'number' => '', 'posttype' => '', 'parentid' => '' ) );
			...
			?>
			<p>
				<label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Posts to display:' ) ?></label> <em>(keep empty to display all posts)</em>
...
/* GENERATE SHORT CODE */
add_shortcode('listposts', 'shortcode_listposts');

function shortcode_listposts( $atts ) {
