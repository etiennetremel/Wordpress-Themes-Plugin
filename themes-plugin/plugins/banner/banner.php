...
*/
	...
	add_filter( 'enter_title_here', 'banner_change_title' );
	
	function banner_change_title( $title ) {
	...
	function banner_meta_box(){
		add_meta_box( 'banner-items', 'Banner Items', 'banner_metas', 'banner', 'normal', 'low' );
	}  
	
	function banner_metas( $post ){
		global $post;
		$post_id = $post->ID;
	...
	add_action( 'save_post', 'save_banner' ); 

	function save_banner( $post_id ){
		if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) return $post_id;
	
		//Security check, data comming from the right form:
		if ( ! isset( $_POST['banner_info_nonce'] ) || ! $_POST['banner_info_nonce'] = 'banner_info_nonce' . $post_id ) return $post_id;
	...
	add_action( 'manage_banner_posts_custom_column',  'banner_custom_columns', 10, 2 ); 

	function banner_custom_columns( $col, $post_id ){
		$items = get_post_meta( $post_id, 'banner', true );
	...
	add_action( 'admin_init', 'register_banner_admin_scripts' );
	function register_banner_admin_scripts() {
	...
	function enqueue_banner_admin_scripts() {
		wp_enqueue_script( 'banner_admin_script' );
		wp_enqueue_style( 'banner_admin_style' );
		wp_enqueue_style( 'thickbox' );
	}

	add_action( 'wp_enqueue_scripts', 'enqueue_banner_frontend_scripts' );
	function enqueue_banner_frontend_scripts() {
