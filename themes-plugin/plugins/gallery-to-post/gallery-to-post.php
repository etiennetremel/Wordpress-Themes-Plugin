...
*/
	...
	add_action( 'add_meta_boxes', 'gallery_to_post_meta_box' );   

	function gallery_to_post_meta_box(){
		add_meta_box( 'gallery_to_post-items', 'Gallery Items', 'gallery_to_post_metas', 'page', 'normal', 'low' );
		add_meta_box( 'gallery_to_post-items', 'Gallery Items', 'gallery_to_post_metas', 'post', 'normal', 'low' );
	}  
	
	function gallery_to_post_metas( $post ){
		global $post;
	...
	add_action( 'save_post', 'save_gallery_to_post' ); 

	function save_gallery_to_post( $post_id ){
		if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) return $post_id;
	
		//Security check, data comming from the right form:
		if ( ! isset( $_POST['gallery_to_post_info_nonce'] ) || ! $_POST['gallery_to_post_info_nonce'] = 'gallery_to_post_info_nonce' . $post_id ) return $post_id;
	...
	add_action( 'admin_init', 'register_gallery_to_post_admin_scripts' );
	function register_gallery_to_post_admin_scripts() {
	...
	add_action('admin_enqueue_scripts', 'enqueue_gallery_to_post_admin_scripts' );
	function enqueue_gallery_to_post_admin_scripts() {
	...
	/* GENERATE SHORT CODE */
	add_shortcode('gallery_to_post', 'shortcode_gallery_to_post');

	function shortcode_gallery_to_post( $atts ) {
		global $post;
